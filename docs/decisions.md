...
Why an ambiguous, contradictory or silent source document was resolved the way it was. This is the
record that stops a later session "fixing" a deliberate choice, and it is the reason this project
can answer *why* as well as *what*.

Numbering is unchanged from when these lived in `README.md`, so every existing citation — in source
comments, in `CHANGELOG.md`, in `HANDOFF.md`, in `apps/web/scripts/bundle-budget.mjs` — still
resolves. **Do not renumber.** Append.

Some items describe behaviour that has since been retired or renamed; those carry a marker saying
so rather than being rewritten, because the decision is the historical record and editing it would
destroy the thing being recorded.
...
61. **Revised V0.6 slice 9 (table aids) made seven calls the meeting asks leave open.** Source:
