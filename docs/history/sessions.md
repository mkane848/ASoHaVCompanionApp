...
`HANDOFF.md`'s chained "Previously (Nth session)" narrative, sixty-two sessions deep, moved here
verbatim. It sat *above* "Current state" in that file, so every session read all of it before
reaching the part it needed.

Kept because the reasoning in it is frequently the only record of why something was done — but it
is history, and nothing here is guaranteed to describe current behaviour.

_For what is true now, read `HANDOFF.md`'s "Current state" and "Open issues"._
