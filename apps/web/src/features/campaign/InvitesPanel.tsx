    ...
    <div style={{ background: 'var(--panel)', border: '1px solid var(--rule)', padding: '16px 18px' }}>
      ...
      {invites.map((i) => (
        <div key={i.Id} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 10, padding: '9px 0', borderTop: '1px solid var(--rule-soft)' }}>
          <span style={{ flex: 1, minWidth: 160, fontSize: 13.5 }}>{i.Email}</span>
          ...
          <button onClick={() => onRevoke(i.Id)} style={{ background: 'transparent', border: 'none', color: 'var(--ink-38)', fontSize: 17, lineHeight: 1, padding: '0 4px' }}>&times;</button>
        </div>
      ))}
