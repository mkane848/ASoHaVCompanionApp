...
campaignRouter.post('/', wrap(async (req, res) => {
  ...
  const party: Party = { Id: newId('pt'), CampaignId: campaign.Id, Rapport: 0, Misfortune: 1, RapportImprovementsTaken: [], History: [], PartyLevel: 0, Motif: '', Quest: '', SkillTags: [], WeaknessTags: [], Path: '', Goal: '', CampAssets: [], UpdatedAt: nowIso(), UpdatedBy: null };
...
campaignRouter.get('/:id/bootstrap', wrap(async (req, res) => {
  ...
  if (!party) {
    ...
    party = { Id: newId('pt'), CampaignId: campaign.Id, Rapport: 0, Misfortune: 1, RapportImprovementsTaken: [], History: [], PartyLevel: 0, Motif: '', Quest: '', SkillTags: [], WeaknessTags: [], Path: '', Goal: '', CampAssets: [], UpdatedAt: nowIso(), UpdatedBy: null };
    await saveParty(party);
  }
  let world = worldRow;
  if (!world) {
