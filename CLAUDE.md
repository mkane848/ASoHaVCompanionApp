...
`0.51.0`, the second of three releases out of a full project audit. `0.50.0` was correctness and
safety with no new scope — the unguarded boot seed that caused the `0.28.0` outage, optimistic
locking on the content library, `CharacterSheet.Hold`'s missing spend path, four routes that
skipped the archive freeze, and three JSONB aggregates with no read-time normalize. `0.51.0` is
Content Admin: destructive actions confirmed, unsaved edits guarded, `required`/`default` finally
read, and the standard CRUD the panel never had (search across the record, schema-derived filters,
sort, duplicate, restore-a-delete, a referential-integrity guard, deep-linkable views) — see
"Architecture: Content Admin" below. A **docs split-and-trim is the agreed third and last**
(`0.52.0`). See `CHANGELOG.md` and `HANDOFF.md`'s header for what else the audit found. Ruleset **V0.6** was adopted 2026-09-09 and is now canonical (`Planning Docs/
...
controls. **Slice 4 — Moves and Camp content — shipped in `0.45.0`**: all 22 seeded Moves
...
superseded by V0.6 slice 4 (`0.45.0`): see "Architecture: Moves and Camp content" above.** The
